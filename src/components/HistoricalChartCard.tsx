...
function calculateSMA(data: number[], period: number): Array<number | null> {
  return data.map((_, index) => {
    if (index < period - 1) return null;

    const window = data.slice(index - period + 1, index + 1);
    const sum = window.reduce((acc: number, value: number) => acc + value, 0);
...
function calculateRSI(
  ...
  const rsi: Array<number | null> = Array(data.length).fill(null);
...
export default function HistoricalChartCard({
  ...
  const sma20Data = useMemo(() => calculateSMA(prices, 20), [prices]);
  const sma50Data = useMemo(() => calculateSMA(prices, 50), [prices]);
  ...
  const chartData = useMemo(() => {
    ...
    if (showSMA50) {
      datasets.push({
        ...
        data: sma50Data,
        borderColor: "#B37DFF",
        backgroundColor: "rgba(179, 125, 255, 0.12)",
        tension: 0.25,
        pointRadius: 0,
        borderWidth: 2,
        yAxisID: "y",
